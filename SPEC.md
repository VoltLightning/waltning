...
| Control | Decision |
|---|---|
| Backup without a backend | An app-owned, `age`-encrypted export the owner controls. **One vendor never holds both halves** (§5.3, `architecture/14-local-first.md` §14.3). **The owner holds the key, on both platforms, and the app stores no copy**: an X25519 identity is generated per export, shown once, and dropped when the screen is dismissed. That claim is about *this app* and stops there — two places outside it hold the key anyway, and both are the platform's: the system clipboard if the owner copies (on iOS that relays through iCloud, so a ciphertext sent to iCloud Drive from the same share sheet puts both halves with one vendor), and the snapshot iOS takes of the screen at backgrounding, which the *Screen capture* row and the launch gate's cover are the controls for. That is the Android answer already — a Keystore key is non-exportable, so an export keyed from it dies with the device it existed to survive — and it is the iOS answer until iCloud Keychain escrow is reachable, which needs a build that is ours (E0). Holding nothing is the stronger position while that is true: a stolen phone yields no backup key, because there is none on it. **The cost is a key per file**, so both carry the same six characters of the recipient — in the filename and beside the key — and pairing a folder of `.age` files with a password manager needs neither opened |
...
| App launch | The UI is gated behind `expo-local-authentication`, tested on `getEnrolledLevelAsync()` and never on `isEnrolledAsync()`. There is no database key to unwrap — the file is protected by class A | The same call, over `BiometricPrompt`, with `biometricsSecurityLevel: 'strong'` where the device has strong biometrics and `'weak'` where it has only class-2 ones — asking for strong on a device that cannot give it is `not_available` every time, and a gate nobody can pass is a lockout; the device credential is the fallback either way. Same enrolment test, and here it is load-bearing rather than tidy: the wrong one locks out a PIN-protected device that is behaving correctly |
...
holds transactions is exactly what archiving is for. What is refused is a
