...
it("100% renders plainly, with no date", () => {
  render(<CoverageTag days={100} realDays={100} calendarDays={100} pct={100} />);
...
it("0% — nothing held yet — says so, never a bare percentage", () => {
  render(<CoverageTag days={0} realDays={0} calendarDays={0} pct={0} />);
...
// "complete" even with just one real quote behind it. `realDays` is the
// decision variable now: a dead source carried every day to today stays
// amber, with the real last-quote date stated.
it("M3 — filled to today by carry, but only realDays decides complete", () => {
  render(<CoverageTag days={10} realDays={1} calendarDays={10} pct={100} lastDate="2026-01-01" />);
  expect(screen.getByText("100% · last quote 2026-01-01")).toBeDefined();
  expect(screen.queryByText("100%")).toBeNull();
});
