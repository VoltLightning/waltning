...
function Probe({ clearBottom }: { clearBottom?: boolean }) {
  const inset = useGroundInset(clearBottom === undefined ? undefined : { clearBottom });
...
/**
 * `GroundPanel`'s own `clearBottom` cannot reach a screen's scroller in
 * `scroll="own"` — the panel has no bottom there to withhold. So the flag has
 * to exist here too, or the first screen to put a `Dock` under a list-owning
 * panel pays the home indicator and the button's room twice: once inside the
 * list, once in the dock below it.
 */
it("adds no device bottom when the panel is not the screen's bottom edge", () => {
  render(
    <FloatingClearanceProvider value={floating.clearance}>
      <SafeAreaProvider insets={NOTCHED}>
        <Probe clearBottom={false} />
      </SafeAreaProvider>
    </FloatingClearanceProvider>,
  );
...
it("leaves more room than a focus ring needs", () => {
  render(<Probe />);
  // Derived, not transcribed: a 4 written here would keep passing if the ring
  // grew, which is the one change that could make this test's subject false.
  expect(read().gutter.paddingLeft).toBeGreaterThanOrEqual(focus.width + focus.offset);
});
