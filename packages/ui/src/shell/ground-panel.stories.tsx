...
export const TallContentStory: Story = {
  ...
  play: async ({ canvasElement }) => {
    ...
    if (!(scroll instanceof HTMLElement)) {
      throw new Error("ground-panel.stories.tsx: no GroundPanel scroll container to scroll");
    }
    ...
    if (scroll.scrollHeight <= scroll.clientHeight) {
      ...
      throw new Error(
        "ground-panel.stories.tsx: the panel did not scroll — scrollHeight <= clientHeight " +
          "in a real browser",
      );
    }
    ...
    if (!(lastRow instanceof HTMLElement)) {
      throw new Error("ground-panel.stories.tsx: no last row to assert against");
    }
    ...
    if (row.bottom > viewport.bottom) {
      throw new Error(
        `ground-panel.stories.tsx: the last row's bottom (${row.bottom}) sits below the ` +
    ...
    if (viewport.bottom - row.bottom < CLEARANCE_FLOOR) {
      throw new Error(
        `ground-panel.stories.tsx: only ${viewport.bottom - row.bottom}px separates the last ` +
