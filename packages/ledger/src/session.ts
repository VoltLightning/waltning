...
export type LocalLedgerSession = {
  ...
  /** Every year the ledger holds something in — S04's year picker. */
  readLedgerYears: () => readonly number[];
  /** The nearest month holding something, for a `range` empty state (§8.1). */
  readNearestActivity: (period: Period) => NearestActivity | null;
...
export function createLocalLedgerSession<TRun>(
  ...
  return {
    ...
    readLedgerYears: () => readLedgerYears(requireOpen().replica.db),
