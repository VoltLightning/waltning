...
| **Quiet day** | One line |
| **Quiet run** | Two or more consecutive: **the same muted line a quiet day is, with the range on it** — *3 – 27 August ——— 25 days · nothing recorded*. Twenty-five days is not twenty-five rows. Nothing to press: it was a bordered card with a *Show* button, the loudest thing in a list of rows that actually hold something, and *Show* led to a run of days with nothing on them — what the row had already said. **After today it is *not yet***: a list led past today (the strip left on next Saturday) splits the run at today — *23 – 26 September ——— 4 days · not yet* above today's own *nothing* line — because *nothing recorded* is a claim about the ledger that is only true of days that have happened, and today folded into a run named by its newer end was a today the pill could not land on. A single empty day after today reads *not yet* the same way |
