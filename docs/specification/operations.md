...
| `set_transaction_lines` | ❌ | The optional breakdown (§10.3) — and, with `amount_original`, the transaction's own amount (§6.10). **Not auto-eligible for that reason**: an operation that can move the fact every balance reads is not a breakdown an agent may write unattended, whatever its name says |
