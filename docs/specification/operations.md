...
| `set_transaction_lines` | ❌ | The optional breakdown (§10.3) — and, with `amount_original`, the transaction's own amount (§6.10). **Not auto-eligible for that reason**: an operation that can move the fact every balance reads is not a breakdown an agent may write unattended, whatever its name says. Enforced by `registry.test.ts`'s "never marks a write auto-eligible", which holds every `kind: "write"` at `false` — this operation has no server-side declaration yet, and that rule is what it will meet when it does |
