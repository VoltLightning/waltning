...
| `allocate_shares` | ❌ | **J08's whole write, and it was missing** — the journey's path names an ALLOCATE step and no operation performed it. Takes the clearing account, the date, one category, a list of `{counterpartyId, weight-or-amount}` and your own share; writes one expense out of the pot per row, each counterparty row carrying `counterparty_role = 'debt'` and your own row carrying none. **One transaction, so a split is one audited event** rather than four rows that might half-exist. §8's largest-remainder arithmetic runs here, over weights, never over amounts a client rounded — and the payer is index 0, which is what makes §5's *"remainder assigned to the payer"* true rather than incidental. Never auto: it creates obligations between people |
