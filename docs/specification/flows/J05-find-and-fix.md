...
| Edit conflicts with another device | Not clock-based. The write carries the version it last read; if that field changed under it, it is a real conflict — same-field divergence follows the conflict setting (latest-applied-wins or ask), and the tax-sensitive fields always ask (`architecture/14` §14.2). The outbox separately carries a client UUID so a retried write cannot become a second row (`SPEC.md` §14.3) |
