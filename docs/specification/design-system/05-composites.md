...
| `BottomSheet` | Bottom-anchored, and **it owns its own height**: at most the window less 170px from the top — or less the device's top inset plus the design padding, whichever leaves less — with a header that does not scroll, a body that always does, and a `footer` slot pinned under it. A caller that passes no footer still gets a bounded, scrolling sheet, because the sheets that needed one never asked. The bottom inset is padding, not a subtraction: the sheet reaches the edge of the screen and its *content* clears the home indicator. A picker that brings its own bounded list lays out inside the body as it always did |
| `TabHeader` | The band a tab root wears when it has no hero to lead with: the tab's own label in `shell`, at `displayTwo`, clearing the top inset the way `Shell` does, with room for **one** right-side action. Drawn by the tab shell from the active tab's label, never by the screen — a screen that draws its own header is a screen that can disagree with the next one, which is how one app came to have three treatments (Today's hero band, nothing at all, and a title inside a card). Today is the exception and keeps its hero: a 54pt total does not fit in a navigation bar, and a hero is a better header than a word |
| `TabBar` | **Four tabs today** — Today · Ledger · Debt · Settings — duotone icons, ≥44px targets. Calendar is registered as a route and unlisted until S11 builds the screen: a fifth of the bar leading to *"this screen isn't built yet"* teaches the other four to be ignored. The count is the caller's, not the component's. Metrics: 8 above the icon, the icon in a fixed 20px box every glyph draws inside, 4 to the label, 8 under it, plus the device's bottom inset — 56pt of bar over whatever the phone reserves, against iOS's own 49pt + inset. **The add button is not in it** — it floats above the whole screen and parks on the bottom edge (`02-tokens` §2.9), so the bar never has to make room for it |
