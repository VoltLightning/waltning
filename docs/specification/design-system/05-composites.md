...
| `BottomSheet` | Bottom-anchored, and **it owns its own height**: at most the window less 170px from the top — or less the device's top inset plus the design padding, whichever leaves less — with a header that does not scroll, a body that always does, and a `footer` slot for the commitment a sheet ends with, pinned so it cannot scroll away. A caller that passes no footer still gets a bounded, scrolling sheet, because the sheets that needed one never asked. The bottom inset is padding, not a subtraction: the sheet reaches the edge of the screen and its *content* clears the home indicator. **The soft keyboard is subtracted twice** — the sheet is lifted by it, so its bottom edge and the pinned footer land on the keyboard's top edge, and the cap shrinks by the same amount so the lift never pushes the sheet's head off the window; the home-indicator inset is dropped while the keyboard is over it. On iOS the window height does not change when the keyboard opens, so a sheet capped against the window alone keeps drawing under it. A picker that brings its own bounded list lays out inside the body as it always did |
