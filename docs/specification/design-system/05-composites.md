...
| `GroundPanel` | `radius-lg` surface lifting over the shell. Clears the bottom and side insets; the top belongs to the header above it. **The page scroller** — every screen scrolls through it; a screen that owns a virtualized list, directly or through a component it renders (`RateTable`'s own `FlatList`, say), passes `scroll="own"` instead, and nothing nests a second scroller. `clearBottom` (default `true`) drops the bottom's device inset — the design padding stays — for a screen whose bottom edge is already owned by something else fixed there (`Dock`, on the transfer and quick-add screens) |
| `Card` | `surface`, `radius-md`, a one-pixel `border`, no shadow; optional title and action. A card groups related rows or holds a hero figure — which screens want one, and how many, is a design decision made per screen |
