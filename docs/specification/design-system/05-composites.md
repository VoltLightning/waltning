...
| `TabBar` | **Three tabs today** — Today · Debt · Settings — duotone icons, ≥44px targets. There is no Ledger tab and no Calendar tab: S04 absorbed both at phone width (S04 §9), so a tab for either would lead to the screen the user is already on. S10 and S11 keep their routes on the desk, where the shell has a nav bar rather than a tab bar. The freed slot stays empty — a bar padded back to four for symmetry teaches the real tabs to be ignored just as surely as a tab leading to *"this screen isn't built yet"* would. The count is the caller's, not the component's. Metrics: 8 above the icon, the icon in a fixed 20px box every glyph draws inside, 4 to the label, 8 under it, plus the device's bottom inset — 56pt of bar over whatever the phone reserves, against iOS's own 49pt + inset. **The add button is not in it** — it floats above the whole screen and parks on the bottom edge (`02-tokens` §2.9), so the bar never has to make room for it |
