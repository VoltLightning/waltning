...
| Focus visible | ✅ | 2px, 2px offset. `focus-ring` on a page fill; `shell-focus-ring` on the band, because the green ring is 2.45:1 there — under the 3:1 a boundary needs, on the four controls a keyboard reaches first. Both are censused against the fills they land on |
