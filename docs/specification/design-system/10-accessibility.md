...
| Focus visible | ✅ | 2px, 2px offset. `focus-ring` on a page fill — censused against all five, since a ring is drawn *while* a control is hovered or pressed, where the old `#6f8f66` sat at 2.89 and 2.69. `shell-focus-ring` on the band, because a green ring is 2.04:1 there. Seven controls draw the band's ring and each asserts it in its own render test — the token census alone could not see the whole fix being deleted, and five of the seven live outside `shell/` |
