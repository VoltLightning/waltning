...
| `RuleHealthTag` | S21 |
| `BackupCard` | S30's Back up screen — the key an `architecture/14` §14.3 export was written to, shown once. The key is the card's hero and is drawn whole, in a monospace face, because it is read character by character and typed elsewhere; the rows beneath state what the file holds. It renders no `<Amount>`, correctly — a backup is measured in entries and kilobytes |
| `EmptyState` variants | `first-run` · `filtered` · `range` |
| `ErrorState` variants | `recoverable` · `terminal` · `partial` |
| `StartupFailed` | The ledger could not open — `_layout.tsx`'s own replacement for the whole app, before any router exists. No `cost` line on either branch: a pre-journal rebuild deletes both stores before it can fail, so no constant claim about what was lost is true everywhere it would render. **Which branch, and which sentence, follows from where the failure happened.** `terminal`, no action, the failing layer's own words verbatim — reached only once the storage engine answered, so the layer that threw is the migrator or the session, both of which write for a person and will refuse the same file again. `recoverable` with **Try again** for the platform never getting that engine up: its files still held by the document being replaced, or a worker that did not answer at all. Those write a browser's untranslated paragraph about an API method, or a timeout, so that branch says a translated sentence of the app's own — one per cause — and the original goes to the development log (`architecture/14` §14.1) |
